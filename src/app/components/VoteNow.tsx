...
export default function VoteNow({ department_org }: { department_org?: string }) {
  ...
  const checkIfUserHasVoted = async (electionId: string) => {
    try {
      ...
      if (!user) return;
      // Debug log
      console.log('Checking for user_id:', user.id, 'election_id:', electionId);
  ...
  console.log('VoteNow rendered. hasVoted:', hasVoted, 'elections:', elections);
  ...
  if (hasVoted && elections.length > 0 && elections[0].is_uni_level) {
    // Show only the blue message and a non-interactive candidates list
    const election = elections[0];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-red-950 to-red-900 flex flex-col">
        ...
        {/* Blue Already Voted Message */}
        <div className="bg-blue-50 border border-blue-300 rounded-lg p-8 text-center m-8">
          <div className="text-blue-700 text-2xl font-bold mb-2">You have already voted in this university election</div>
          <p className="text-blue-600 text-lg">Thank you for participating in the university-wide voting process!</p>
        </div>
        {/* Non-interactive Candidates List */}
        <div className="flex-1 bg-stone-50 overflow-y-auto pointer-events-none opacity-100">
                    ...
                    {positionCandidates.length > 0 ? (
                      <div className="space-y-4">
                        {positionCandidates.map((candidate: Candidate) => (
                          <div
                            key={candidate.id}
                            className="border border-yellow-300 rounded-lg p-4 transition-colors"
                          >
  ...
  console.log('RENDERED: voting UI');
  ...
  if (elections.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-red-950 to-red-900 flex flex-col">
        ...
        {/* Already Voted Message below header */}
        <div className="bg-blue-50 border border-blue-300 rounded-lg p-8 text-center m-8">
          <div className="text-blue-700 text-2xl font-bold mb-2">You have already voted in this university election</div>
          <p className="text-blue-600 text-lg">Thank you for participating in the university-wide voting process!</p>
        </div>
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-b from-red-950 to-red-900 flex flex-col">
        ...
        {/* Voting UI (candidates, etc.) */}
        <div className={hasVoted ? "pointer-events-none" : ""}>
          <div className="flex-1 bg-stone-50 overflow-y-auto">
            <div className="p-4 bg-gradient-to-r from-red-50 to-yellow-50 min-h-full">
              <div className="max-w-7xl mx-auto space-y-6">
                {/* Header Section */}
                                        ...
                                        <div className="space-y-4">
                                          {positionCandidates.map((candidate: Candidate) => (
                                            <div
                                              key={candidate.id}
                                              className={`border border-yellow-300 rounded-lg p-4 transition-colors ${
                                                hasVoted ? 'opacity-50 cursor-not-allowed pointer-events-none' : 'hover:bg-yellow-50'
                                              }`}
                                              onClick={!hasVoted ? () => handleCandidateSelect(position.id, candidate.id) : undefined}
                                            >
                                              <div className="flex items-start gap-4">
                                                <div className="mt-1">
      ...
      <div ref={candidatesSectionRef}>
        {/* ...candidates listing code (the section that lists all candidates)... */}
      </div>
    </>
  );
} 